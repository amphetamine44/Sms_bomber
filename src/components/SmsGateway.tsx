...
const PROVIDERS: Provider[] = [
  ...
  // ====== NEW PROVIDERS (from Python script) ======
  {
    id: 'confirmtkt',
    name: 'ConfirmTkt',
    countries: ['India'],
    ...
    bodyTemplate: () => ({}),
    requires: [],
    identifier: '"success"', // adjust based on real response
  },
  {
    id: 'justdial',
    name: 'Justdial',
    countries: ['India'],
    endpoint: (creds) =>
      `https://t.justdial.com/api/india_api_write/10aug2016/sendvcode.php?mobile=`,
    method: 'GET',
    headers: {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    },
  ...
  const send = async (providers: Provider[], phone: string, message: string, creds: Record<string, string>) => {
    ...
    for (const provider of providers) {
      try {
        ...
        // For new providers – no extra fields

        const endpoint = typeof provider.endpoint === 'function'
          ? provider.endpoint(providerCreds)
          : provider.endpoint;

        // For GET endpoints that use query parameters, append the phone number directly
        let finalEndpoint = endpoint;
        if (provider.method === 'GET' && endpoint.includes('?') && !endpoint.includes('Replace')) {
          // The endpoint already ends with '=' or '?', we append the phone
          // but we need to ensure the phone is appended correctly.
          // If endpoint is a function that returns the full URL with the phone already appended,
          // we can just use it as is. But our functions above return the base URL with '='.
          // We'll append the phone here.
          if (!finalEndpoint.endsWith('=') && !finalEndpoint.endsWith('&')) {
            finalEndpoint += '=';
          }
          finalEndpoint += phone;
        }
...
export default function SmsGateway() {
  ...
  const handleGenerateCurl = () => {
    ...
    let cmd = `curl -X ${provider.method} '${finalEndpoint}' \\\n`;
